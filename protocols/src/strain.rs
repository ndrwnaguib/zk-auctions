...
use zk_auctions_core::gm::{
	StrainRandomGenerator, dot_mod, embed_and, encrypt_bit_gm_coin,
};
...
impl Default for StrainConfig {
    fn default() -> Self {
        Self {
            challenges_length: 40,
            bit_length: 32,
            default_dlog_iters: 10,
            modpow_exponent: 4,
        }
    }
}
...
/// Trait defining the Strain protocol operations
pub trait Strain {
	/// Compute proof of encryption
	fn compare_proof_enc(&self, c1: Vec<BigInt>, n1: &BigInt, r1: &[BigInt]) -> Vec<Vec<Vec<BigInt>>>;
...
pub struct StrainProtocol {
    config: StrainConfig,
}

impl Default for StrainProtocol {
    fn default() -> Self {
        Self {
...
impl StrainProtocol {
    /// Create a new StrainProtocol with custom configuration
    pub fn new(config: StrainConfig) -> Self {
        Self { config }
    }
}

impl Strain for StrainProtocol {
	fn compare_proof_enc(&self, c1: Vec<BigInt>, n1: &BigInt, r1: &[BigInt]) -> Vec<Vec<Vec<BigInt>>> {
