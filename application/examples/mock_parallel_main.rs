...
use zk_auctions_application::mock_auctioneer_thread::{AuctioneerThread, create_auction_channels};
use zk_auctions_application::mock_bidder_thread::BidderThread;
...
fn main() {
    // Generate random bids for bidders
    let mut rng = rand::thread_rng();
    
    // Example: 2 bidders working in parallel with random bids
    let bidder_configs = vec![
        ParallelBidderConfig { 
            id: 0, 
            name: "Alice".to_string(), 
            bid_value: rng.gen_bigint_range(&BigInt::from(0u32), &(BigInt::from(1u32) << 31))
        },
        ParallelBidderConfig { 
            id: 1, 
            name: "Bob".to_string(), 
