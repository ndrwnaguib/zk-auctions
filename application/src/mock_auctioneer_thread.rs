...
use zk_auctions::strain::mock_auctioneer::MockAuctioneerHost;
...
/// The `AuctioneerThread` struct represents the auctioneer in a multi-bidder auction protocol.
/// 
/// This thread is responsible for coordinating the auction process, including:
/// - Managing the security parameters for the protocol.
/// - Sending messages to all participating bidders via their respective channels.
/// - Receiving and processing messages from bidders, such as join notifications, public information, proofs, and public keys.
/// 
/// # Fields
/// - `soundness`: Security parameters for the auction protocol.
/// - `bidder_txs`: A vector of channels to send messages to each bidder.
/// - `rx`: The channel to receive messages from bidders.
pub struct AuctioneerThread {
    /// Security parameters for the protocol.
    pub soundness: StrainSecurityParams,
    /// Channels to send messages to each bidder.
    pub bidder_txs: Vec<mpsc::Sender<AuctionMessage>>,
    /// Channel to receive messages from bidders.
...
impl AuctioneerThread {
    ...
    // TODO: num_bidders should be determined by a time limit
    pub fn run(mut self, num_bidders: usize) -> ParallelAuctionResult {
        let tag = "Auctioneer";
        tprintln!(tag, "Starting auction with {} bidders", num_bidders);

        let mut auctioneer_host = MockAuctioneerHost::new(self.soundness);
        ...
        tprintln!(tag, "⏳ 2.2: Waiting for proofs to be received...");
            ...
            if let Ok(msg) = self.rx.recv() {
                match msg {
                    AuctionMessage::ProofGenerated { prover_id, target_id, receipt, private_output } => {
                        ...
                        tprintln!(
                            tag,
                            "🔒 2.2: Received proof from bidder {} against bidder {} ({}/{})",
