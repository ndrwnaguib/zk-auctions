...
fn main() {
    ...
    let is_eval_verified = match eval_verification {
        Some(_) => {
            eprintln!(
                "r0vm-strain-auction:auctioneer-verify: Evaluation proof verified successfully."
            );
            true
        }
        None => {
            eprintln!(
                "r0vm-strain-auction:auctioneer-verify: Evaluation proof verification failed."
            );
            false
        }
    };
    ...
    eprintln!(
        "r0vm-strain-auction:auctioneer-verify: Committing verification result: {}",
        is_eval_verified
    );
