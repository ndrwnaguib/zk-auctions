...
fn main() {
    ...
    for (idx, other_bidder_receipt) in other_bidder_receipts.into_iter().enumerate() {
        eprintln!("r0vm-strain-auction:bidder-verify: Decoding receipt journal for other bidder {}...", idx);
        ...
        eprintln!("r0vm-strain-auction:bidder-verify: Performing compare_leq_honest for n_j = {n_j}.");
    ...
    if is_winner {
        eprintln!("r0vm_strain_auction: BIDDER_VERIFY: Current bidder WON. Committing Some(v_i = {v_i}).");
        env::commit(&Some(v_i));
    } else {
